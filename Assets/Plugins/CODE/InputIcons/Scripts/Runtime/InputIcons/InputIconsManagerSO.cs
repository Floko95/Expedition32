    ...
    public class InputIconsManagerSO : ScriptableObject
    {
        ...
        public static InputIconsManagerSO Instance
        {
            get
            {
                if (instance != null)
                    return instance;
                InputIconsManagerSO[] resourceManagers = Resources.LoadAll<InputIconsManagerSO>("InputIcons");
                // 1. Check resources for actual manager
                instance = resourceManagers.FirstOrDefault(m => m.isActualManager);
                if (instance != null) return instance;
                // 2. Check outside resources for actual manager
                InputIconsManagerSO[] allManagers = FindAllManagers();
                Debug.Log("managers found total: " + allManagers.Length);
                instance = allManagers.FirstOrDefault(m => m.isActualManager);
                if (instance != null)
                {
                    Debug.LogWarning("InputIconsManager with isActualManager found outside Resources/InputIcons/ folder. Please move it to a Resources folder.");
                    return instance;
                }
                // 3. Try any manager from resources
                if (resourceManagers.Length > 0)
                {
                    instance = resourceManagers[0];
                    return instance;
                }
                // 4. No manager found
                Debug.LogWarning("No InputIconsManager found.");
                return null;
            }
            set => instance = value;
        }
        ...
        private void Awake()
        {
            if (Instance == null && isActualManager)
                Instance = this;
        }
        ...
        {
            inputStyleKeyboardDataList.Clear();
            inputStyleGamepadDataList.Clear();
        }
        ...
        {
            return Instance ? Instance.currentInputDevice : null;
        }
        ...
        {
            return Instance.currentInputDevice is Gamepad;
        }
        ...
        {
            if(overrides == null)
                overrides = InputIconsManagerSO.GetSavedBindingOverrides();
