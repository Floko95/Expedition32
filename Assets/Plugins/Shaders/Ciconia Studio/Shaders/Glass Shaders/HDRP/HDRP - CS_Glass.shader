Shader "Ciconia Studio/CS_Glass/HDRP/Glass"
{
	Properties
	{
		...
		[Space(45)][Header(Fade Properties________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________)][Space(15)]_Fade("Fade", Range( 0 , 1)) = 0.2
	...
	SubShader
	{
		...
		Pass
		{
						...
						)
			{
				...
				float4 tex2DNode76 = tex2D( _MainTex, ( appendResult14_g23 + appendResult13_g23 ) );
				...
				float2 break26_g26 = uv_BumpMap;
				float2 appendResult14_g26 = (float2(( break26_g26.x * GlobalTilingX281 ) , ( break26_g26.y * GlobalTilingY282 )));
				float2 appendResult13_g26 = (float2(( break26_g26.x + GlobalOffsetX541 ) , ( break26_g26.y + GlobalOffsetY540 )));
				float3 unpack2 = UnpackNormalScale( tex2D( _BumpMap, ( appendResult14_g26 + appendResult13_g26 ) ), _BumpScale );
				...
				#endif
				float4 temp_output_169_0 = ( ( ( ifLocalVar470 * ( _FresnelStrength * 200 ) ) * staticSwitch461 ) + ( texCUBENode6 * ( ( texCUBENode6.a * _ReflectionIntensity ) * 200 ) * _ColorCubemap ) );
				...
				float4 fetchOpaqueVal381 = ASEHDSampleSceneColor(( (ase_grabScreenPosNorm).xyzw + float4( (( ( Normal101 + mul( float4( normalizedWorldNormal , 0.0 ), UNITY_MATRIX_V ).xyz ) * (-1.0 + (_Refraction - 0.0) * (1.0 - -1.0) / (2.0 - 0.0)) )).xyz , 0.0 ) ).xy, 0.0, GetInverseCurrentExposureMultiplier());
				...
				float2 break26_g24 = uv_MetallicGlossMap;
				float2 appendResult14_g24 = (float2(( break26_g24.x * GlobalTilingX281 ) , ( break26_g24.y * GlobalTilingY282 )));
				float2 appendResult13_g24 = (float2(( break26_g24.x + GlobalOffsetX541 ) , ( break26_g24.y + GlobalOffsetY540 )));
				float4 tex2DNode123 = tex2D( _MetallicGlossMap, ( appendResult14_g24 + appendResult13_g24 ) );
				...
				float4 lerpResult396 = lerp( ( Cubmap179 + ( GrabSreenRefraction385 * ( 1.0 - Opacity87 ) ) ) , Cubmap179 , DecalMask182);
				...
				float2 break26_g25 = uv_OcclusionMap;
				float2 appendResult14_g25 = (float2(( break26_g25.x * GlobalTilingX281 ) , ( break26_g25.y * GlobalTilingY282 )));
				float2 appendResult13_g25 = (float2(( break26_g25.x + GlobalOffsetX541 ) , ( break26_g25.y + GlobalOffsetY540 )));
				float blendOpSrc136 = tex2D( _OcclusionMap, ( appendResult14_g25 + appendResult13_g25 ) ).r;
				...
				float fresnelNdotV515 = dot( normalWS, V );
		...
		Pass
		{
			...
			UNITY_SETUP_INSTANCE_ID(packedInput);
				...
				float lerpResult513 = lerp( -3.0 , 0.0 , _GradientFade);
				float fresnelNdotV515 = dot( ase_worldNormal, V );
				float fresnelNode515 = ( lerpResult513 + _PowerFalloffFade * pow( max( 1.0 - fresnelNdotV515 , 0.0001 ), (( 1.0 + -( 1.0 - _Fade ) ) + (1.0 - 0.0) * (( 1.0 - _Fade ) - ( 1.0 + -( 1.0 - _Fade ) )) / (1.0 - 0.0)) ) );
				float clampResult518 = clamp( (( _InvertFresnelFade )?( ( 1.0 - fresnelNode515 ) ):( fresnelNode515 )) , 0.0 , 1.0 );
		...
		Pass
        {
					...
					)
			{
				...
				ase_worldViewDir = normalize(ase_worldViewDir);
				float lerpResult513 = lerp( -3.0 , 0.0 , _GradientFade);
				float fresnelNdotV515 = dot( packedInput.normalWS, ase_worldViewDir );
				float fresnelNode515 = ( lerpResult513 + _PowerFalloffFade * pow( max( 1.0 - fresnelNdotV515 , 0.0001 ), (( 1.0 + -( 1.0 - _Fade ) ) + (1.0 - 0.0) * (( 1.0 - _Fade ) - ( 1.0 + -( 1.0 - _Fade ) )) / (1.0 - 0.0)) ) );
				float clampResult518 = clamp( (( _InvertFresnelFade )?( ( 1.0 - fresnelNode515 ) ):( fresnelNode515 )) , 0.0 , 1.0 );
				float2 uv_DetailAlbedoMap = packedInput.ase_texcoord3.xy * _DetailAlbedoMap_ST.xy + _DetailAlbedoMap_ST.zw;
				float cos400 = cos( radians( _Rotation ) );
